...
# CrCb 살색 히스토그램 구하기
ref = cv2.imread(path + 'kids1.png', cv2.IMREAD_COLOR)
mask = cv2.imread(path + 'kids1_mask.bmp', cv2.IMREAD_GRAYSCALE)        # 직접 작업하여 살색부분만 나타낸 data
...
# 입력 영상에 히스토그램 역투영 적용
src = cv2.imread(path + 'kids2.png', cv2.IMREAD_COLOR)
