...
def drawROI(img, corners):
    ...
    for pt in corners:
        cv2.circle(cpy, tuple(pt.astype(int)), 25, c1, -1, cv2.LINE_AA)      # 바운딩 박스 각 모서리 색과 모양
...
# 입력 이미지 불러오기
src = cv2.imread(path + 'scanned.jpg')
...
# 입력 영상 크기 및 출력 영상 크기
h, w = src.shape[:2]    #(800, 600)
dw = 500
dh = round(dw * 297 / 210)  # A4 용지 크기: 210x297cm


# 모서리 점들의 좌표, 드래그 상태 여부
srcQuad = np.array([[30, 30], [30, h-30], [w-30, h-30], [w-30, 30]], np.float32)      # 바운딩 박스 모서리 좌표들
